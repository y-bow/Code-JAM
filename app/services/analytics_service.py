...
def get_teacher_ratings_data(institution_id):
    ...
    for t in teachers:
        avg_rating = db.session.query(db.func.avg(TeacherRating.rating))\
            .filter_by(teacher_id=t.id).scalar() or 0
        ...
        teacher_stats.append({
            ...
            'recent_reviews': TeacherRating.query.filter_by(teacher_id=t.id)
                .order_by(TeacherRating.created_at.desc()).limit(3).all()
        })
...
def process_nomination(nom_id, action, institution_id, user_id):
    ...
    if action == 'approve':
        nom.status = 'approved'
        nom.approved_by = user_id
        nom.decided_at = datetime.utcnow()
        student = User.query.get(nom.student_id)
        student.role = 'class_rep'
        db.session.commit()
        return True, f'Class Rep nomination for {student.name} approved.'
    elif action == 'reject':
        nom.status = 'rejected'
        nom.approved_by = user_id
        nom.decided_at = datetime.utcnow()
        db.session.commit()
        return True, 'Class Rep nomination rejected.'
